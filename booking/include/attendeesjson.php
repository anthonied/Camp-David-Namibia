...
if($_REQUEST['nonquery'] === 'true')
{
	...
	if($oper === 'edit')
	{
		$database->EditMyEvent($_REQUEST[eventid], $id, $_REQUEST['amountpayed'], $_REQUEST['payedinfull']);

		if($_REQUEST['payedinfull'] == '1')
		{
			$userdetail = $database->GetUserInfoAlt($id);
	...
	elseif ($oper === 'del')
	{
		$database->DeleteEvent($_REQUEST[eventid], $id);
	}
	elseif ($oper === 'add')
	{
	...
	$query = "SELECT COUNT(*) AS count FROM ".TABLE_ATTENDEES." a, ".TABLE_USERS." c WHERE (a.users_idusers=c.idusers) AND a.events_idevents='$_REQUEST[eventid]'". $wh;
	...
	$query = "SELECT a.users_idusers, a.events_idevents, a.amountpayed, a.payedinfull, a.servicenumber, c.firstname, c.lastname, c.birthdate, c.cellphone, c.email FROM ".TABLE_ATTENDEES." a, ".TABLE_USERS." c WHERE (a.users_idusers=c.idusers) AND a.events_idevents='$_REQUEST[eventid]'". $wh. " ORDER BY ".$sidx." ".$sord. " LIMIT ".$start." , ".$limit;
